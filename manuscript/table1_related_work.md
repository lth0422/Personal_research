...
개별 논문 49편을 모두 나열하지 않고, 관련연구 계열별 핵심 차이만 보여준다.
...
| Deadline-aware DNN serving | Yao et al.; Xu et al.; Cao et al.; Han et al.; He et al.; Zhang et al.; Raj et al. | DNN stage/depth, exit point, batch, fusion, offloading, edge/cloud placement | Confidence, SLO/deadline, queue/load, GPU budget, network/cloud variability | Edge GPU/server, SLO, on-time ratio, deadline miss, throughput-latency trade-off | DNN serving/perception 중심. Vibration FD의 `W/H/M`, anomaly score trigger, PREEMPT_RT kernel timing은 다루지 않음 |
