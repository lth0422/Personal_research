...
% 작성자: Claude (Claude Code) · v1_2 · 2026-08-02
...
\author{Claude (Claude Code) · v1\_2}
\date{2026-08-02}

\begin{document}
\maketitle

\noindent\textbf{동반 산출물:} \texttt{surveys/survey\_master\_claude\_v1\_2.xlsx} (서베이 종합표), \texttt{manuscript/topic\_specification\_claude\_v1\_2.md}.

\section*{0. 문서의 목적}
\begin{itemize}
\item 이 문서는 학위논문의 \textbf{주제를 구체화}하기 위한 내부 작업 문서다.
\item 체계적으로 검토를 마친 두 축, 실시간 결함 진단과 elastic scheduling의 조사 결과를 정리하고 연구 공백·정식화·실험 설계·남은 과제를 모은다.
\item 적응형 입력(S2)과 실시간 DNN 서빙(S4)은 조사를 보강 중이며 현재 확보한 범위만 표로 반영한다.
\item 초록·본문은 논문 초안에 옮길 수 있는 형태로 작성하되 미검증 수치는 넣지 않는다.
\end{itemize}
...
\noindent 검토 의견 --- 제안하신 \emph{Machine Condition and Slack-aware Elastic Real-Time Vibration Fault Diagnosis}
\begin{itemize}
\item 좋은 점: 짧고 두 트리거와 응용이 드러난다. v1의 긴 제목보다 낫다.
\item 고칠 점: \emph{Elastic Real-Time}은 형용사가 겹쳐 어색하고, \emph{Elastic}이 앞에 오면 스케줄링 이론을 새로 제안하는 논문처럼 읽힌다. 우리는 스케줄링을 \textbf{적용}하는 논문이므로 \emph{Elastic}을 빼는 편이 안전하다. 실제 방법이 모드 선택이므로 \emph{Mode Selection}을 넣으면 기여가 분명해진다(제목 관례: [18], [19]).
\end{itemize}
...
\section{국문 초록}
\begin{itemize}
\item 회전 기계의 진동 결함 진단을 엣지에서 실시간으로 수행할 때, 더 큰 입력 window와 더 무거운 모델은 결함을 더 잘 구분하지만 실행시간과 이용률을 함께 키워 데드라인을 넘길 수 있다.
\item 본 연구는 입력 window $W$, 진단 주기를 정하는 hop $H$, 추론 모델 $M$을 하나의 진단 모드로 묶고, 기계 상태 $q$와 시스템 여유 $S$를 함께 기준으로 삼아 매 진단 시점에 모드를 고르는 방법을 제시한다.
\item 정상 상태에서는 가벼운 모드로 여유를 확보하고, 이상 징후가 감지되면 정밀 모드를 고르되 데드라인 안에서 실행 가능한 모드만 후보로 둔다.
\item 평가 플랫폼은 Raspberry Pi Zero 2W이며, 검증 실험으로 일반 Linux와 PREEMPT\_RT의 응답시간 특성을 비교한다.
\end{itemize}

\section{Abstract}
\begin{itemize}
\item In real-time vibration fault diagnosis on edge devices, a larger window $W$ and a heavier model $M$ separate faults more reliably but increase execution time and utilization, which can violate the deadline.
\item This work groups $W$, the hop $H$ that sets the diagnosis period, and $M$ into a single diagnosis mode, and selects one mode at each step using both the machine condition $q$ and the system slack $S$.
\item Under a normal condition it chooses a light mode to preserve slack; under a suspicious condition it chooses a precise mode, restricting candidates to modes that run within the deadline.
\item The platform is a Raspberry Pi Zero 2W; a verification experiment compares stock Linux and PREEMPT\_RT.
\end{itemize}
...
$C_i$ & 추론 실행시간. 분포로 보고(평균·p99·최대) \\
$I,\,L$ & 간섭 시간, 운영체제 스케줄링 지연 \\
$R_i$ & 관측 응답시간, $R_i=C_i+I+L$ \\
$D$ & 상대 데드라인 \\
$U_i,\,U_{bg},\,U_{bound}$ & 모드 이용률 $U_i=C_i/T_i$, 배경 이용률, 이용률 상한 \\
$q_k$ & 기계 상태 지표(machine condition). 본 연구에서는 anomaly score \\
$S_k$ & 시스템 여유, $S_k=D-\mathrm{p99}(\text{최근 응답시간})$ \\
$Q(m_i,q_k)$ & 현재 기계 상태에서 모드 $m_i$의 진단 효용(정확도 또는 검출 지연) \\
...
\subsection{연구 동기}
\begin{itemize}
\item 진동 결함 진단은 엣지에서 두 요구가 부딪힌다. 진단 신뢰성(긴 window·무거운 모델이 약한 결함을 더 잘 구분)과 실시간성(데드라인 준수).
\item KCC 2026 결과가 이 충돌을 정량적으로 보여준다 [22]. 사용한 FRFconv-TDSNet 모델은 depthwise convolution 연산량이 window 크기의 제곱 $O(W^2)$에 비례하는 부분이 있어, window를 줄이면 latency가 선형보다 빠르게 감소했다.
\end{itemize}
\begin{center}\footnotesize
\begin{tabular}{@{}rrrr@{}}
\toprule
$W$ & 최대 실행시간 & 주기 $T$ (8\,kHz) & 이용률 $U=C/T$ \\ \midrule
...
\end{center}
\begin{itemize}
\item 해석: 정밀한 모드로 갈수록 $C$가 커지고, $T$가 충분히 커지지 않으면 $U>1$이 되어 단일 태스크만으로도 데드라인을 지킬 수 없다.
\item 착안: 정상일 때는 가벼운 모드로 여유를 확보하고, 이상 시 정밀 모드로 전환하되 데드라인 안에서 실행 가능한 경우에만 허용한다. 조절 대상은 window 하나가 아니라 $(W,H,M)$을 묶은 모드다.
\item KCC 모델은 분류 형태이나 내부적으로 anomaly detection을 수행하며 그 anomaly score가 $q$가 된다 [22].
\end{itemize}

\subsection{관련 연구 --- 실시간 결함 진단 (검토 완료)}
\begin{itemize}
...
\begin{tabular}{@{}cL{2.4cm}L{2.6cm}L{1.6cm}L{4.2cm}cL{3.2cm}@{}}
\toprule
\# & 논문 & 문제 상황 & 조절 대상 & 적응·이상감지 판정 기준 & DL & 본 연구와의 차이 \\ \midrule
...
[6] & Arciniegas 2025, Discover IoT & 모터 진동 이상 알림 & 없음 & K-means 정상군 거리 임계 초과 시 alert & $\triangle$ & 이상감지가 alert 조건일 뿐 모드 선택 아님 \\
...
\end{center}
\begin{itemize}
\item 이상감지 정량 기준: 통계 관리한계(SPE [1])나 클러스터 거리 임계(K-means [6]). 둘 다 본 연구의 $q$ 임계 설계에 참고가 된다.
\item Lima [2] 표현 교정: 미세 결함이 더 높은 $f_s$·짧은 hop을 ``요구''한 것이 아니라, grid search 결과 그런 설정이 최적으로 \textbf{선택}되었다.
\item Yang [4] 트리거 상세: 단말이 경량 stacked autoencoder를 실행하고, 그 재구성 신뢰도와 허용 latency 예산을 함께 판단해 무거운 진단을 엣지로 오프로딩할지 결정한다. $q+S$와 형태가 가깝지만 스케줄 가능성 분석은 없다.
\end{itemize}

\subsection{관련 연구 --- Elastic Scheduling (검토 완료)}
\begin{itemize}
\item 이론과 적용으로 나눈다(전체 표는 엑셀 시트 2).
...
\subsection{관련 연구 --- 적응형 입력·모델, 실시간 DNN 서빙 (보강 중)}
\begin{center}\scriptsize
\begin{tabular}{@{}cL{2.6cm}L{2.6cm}L{4.4cm}L{1.6cm}@{}}
\toprule
\# & 논문 & 조절 대상 & 조절 기준 & 시점 \\ \midrule
[15] & Kim (ADW) 2026, Mathematics & window $W$ ($H=\beta W$) & anomaly deviation score(변동성·주기·국부 스파이크 편차) & offline \\
[16] & Tang (AIL) 2023, ASC & input length & 물리식(특성 주파수·$f_s$·회전속도) & offline \\
[17] & Tang (AILWTLN) 2023, EAAI & input length + 경량 모델 & 물리식 $N_a=n^2 f_s/BW$ & offline \\
\bottomrule
\end{tabular}
\end{center}
\begin{center}\scriptsize
...
[21] & Yao (Imprecise) 2020, RTCSA & 실행 depth(필수+선택 단계) & 데드라인 + 선택 단계 효용 & imprecise computation + EDF stage 스케줄 \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{연구 공백}
\begin{itemize}
\item 진단 축: 진동 결함 진단에서 runtime에 $W,H,M$을 함께 조절하고 데드라인·스케줄 가능성을 결합한 사례가 서베이 범위에서 확인되지 않았다.
\item 스케줄링 축: elastic scheduling은 정확도를 바꾸는 변수를 다루지 않고 트리거가 시스템 내부나 offline에 한정된다.
\item 결합 축: $q$와 $S$를 분리된 역할로 사용해 정확도를 바꾸는 실행 가능한 모드를 고르고 스케줄 가능성을 제시한 사례가 서베이 범위에서 확인되지 않았다. 본 연구는 이 결합을 대상으로 한다.
\end{itemize}
...
\begin{itemize}
\item 실행시간 $C_i$는 단일 값이 아니라 분포로 보고한다(후보군: 평균·p99·최대).
\item PREEMPT\_RT에서 static hard WCET를 증명 없이 주장하지 않고, 실측 응답시간 기준을 쓴다.
\end{itemize}
\begin{equation}
R_i = C_i + I + L,\qquad R_i(\mathrm{p99}) \le D,\qquad U_i=\frac{C_i}{T_i},\qquad U_{total}(m_i)=U_{bg}+U_i.
\end{equation}

\subsection{실행 가능 조건과 선택 정책}
\begin{itemize}
\item 첫 연구 질문: 이상 시 정밀 모드로 전환해도 정적으로 스케줄 가능한가, 그 보장을 어떻게 제시하는가.
\item 정책은 실행 가능한 모드 집합 안에서만 진단 효용이 큰 모드를 고른다.
\end{itemize}
\begin{align}
F(k) &= \Big\{\, m_i \;:\; U_{bg}+\tfrac{C_i(\mathrm{p99})}{T_i}\le U_{bound}\ \text{ 그리고 }\ R_i(\mathrm{p99})\le D \,\Big\},\\
m^{*}_k &= \arg\max_{m_i \in F(k)}\; Q(m_i, q_k).
\end{align}
\begin{itemize}
\item $m^{*}_k$는 $k$ 시점에 선택된 하나의 모드이며 다른 모드를 포함한다는 뜻이 아니다.
\item $Q$는 정확도 또는 검출 지연으로 측정하며, 실시간 추론 문헌의 utility·reward 정식화 [19, 21]에 대응한다.
\item $F(k)$가 비면 사전 정의 fallback 모드를 실행한다. 데드라인 miss 후 재실행은 그 자체로 timing 보장이 아니므로, 재실행에 필요한 시간을 미리 확보한 경우에만 둔다.
\item 여유는 평균이 아니라 $S_k=D-\mathrm{p99}(\text{최근 응답시간})$으로 정의한다.
\end{itemize}

\subsection{기계 상태와 이상감지 기준}
\begin{itemize}
...
\item $q$는 진단 효용 순위를, $S$는 실행 가능한 모드를 정한다. 두 신호의 역할 분리가 정책의 핵심이다.
\end{itemize}

\subsection{알고리즘 (offline / online)}
\noindent\textbf{Offline}
\begin{enumerate}
\item 후보 모드 집합 구성($W$ 후보 + 물리식 하한).
\item 각 모드 실행시간 $C_i$ 측정(평균·p99·최대).
...
\item 일반 Linux와 PREEMPT\_RT 비교는 \textbf{주 실험이 아니라 검증 실험}이다. 응답시간·jitter 특성을 파악해 실행시간 모델 신뢰성을 확인한다. cyclictest 분포로 PREEMPT\_RT 실효성을 검증한다.
\end{itemize}
...
제안 ($q+S$) & O & O & 실행 가능한 모드 안에서 진단 효용 최대 모드 선택 \\
\bottomrule
\end{tabular}
\end{center}
...
\end{tabular}
\end{center}
\noindent 임계는 검증 데이터 percentile로 보정한다. 세 구간을 오가는 anomaly score 궤적을 재생해 정책의 모드 열·진단 효용·데드라인 miss를 baseline과 비교한다.

\subsection{측정 지표}
\begin{itemize}
\item 실시간성: 데드라인 miss 비율, p99·최대 응답시간, activation jitter, 이용률.
\item 진단: 정확도 또는 검출 지연.
\item 정책 효과: 제안 정책이 baseline 대비 데드라인 miss를 늘리지 않으면서 진단 효용을 개선하는지.
...
\section{Novelty}
\begin{itemize}
\item 본 연구의 위치는 두 축이 만나는 빈 교차점이다. 트리거 축에서 외부 기계 상태 트리거, 조절 대상 축에서 정확도를 바꾸는 변수 --- 두 자리가 서베이 범위에서 확인되지 않았다.
\end{itemize}
\noindent\textbf{세 가지 차별점}
\begin{enumerate}
\item 조절 대상: 기존은 주기·rate·입력 크기 중 하나 [9--21]. 본 연구는 $W,H,M$ 세 축을 함께 고르며 특히 $W$와 $H$를 분리한다.
\item 트리거: 기존은 시스템 내부 [13, 14] 또는 offline [12]이거나 조건 하나만 본다(slack만 [18], 생리 상태만 [19], 이상감지만 [1]). 본 연구는 $q$와 $S$를 분리된 역할로 함께 쓴다.
\item 도메인·보장: 진동 결함 진단에서 실행 가능한 모드 집합 안에서 선택하고 Pi Zero 2W(Linux, PREEMPT\_RT) 실측 응답시간으로 스케줄 가능성을 확인한다.
\end{enumerate}
\noindent\textbf{최근접 논문 대비}
\begin{itemize}
\item DNN-SAM [18]: slack으로 입력 fidelity를 고르나 단일 트리거·공간 vision. 본 연구는 시간축 window와 기계 상태를 함께 쓴다.
\item AMS [19]: 상태로 모델을 고르나 생리 신호이며 시스템 여유 미고려. 본 연구는 여유로 실행 가능성을 제한한다.
\item Langarica [1]: 이상감지로 무거운 추론을 gate하나 여유가 없어 모드 선택이 아니다.
\item ADW [15]: 이상 편차로 $W$를 정하나 offline이다.
\end{itemize}

\section{To-Do}
\begin{itemize}
\item 서베이 보강: 적응형 입력(S2)·실시간 DNN 서빙(S4) 조사 완결 후 엑셀 시트 3, 4와 6.4절 갱신.
\item 정식화 확정: 진단 효용 $Q$ 정의(정확도/검출 지연/$d_{safe}$ [12] 중 우리 상황에 적합한 형태), $C(W,M)$ 가변에 맞는 이용률 bound 유도, 모드 집합·$N$·fallback·hysteresis 확정.
\item 실험: Pi Zero 2W 모드 profiling과 feasibility table, Linux vs PREEMPT\_RT 검증, baseline 5종과 제안 정책 비교.
\item 미결: $q$ 정의, 실행 가능 판정 기준(p95/p99/최대), hysteresis 정도.
\end{itemize}
...
\bibitem{ref22} 이태훈 \emph{et al.}, KCC 2026 선행연구 (STM32F407 + Zephyr RTOS 실시간 추론 성능 분석). 정식 서지정보 확정 필요.
\end{thebibliography}
