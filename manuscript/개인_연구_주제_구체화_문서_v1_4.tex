...
$S_k$ & $k$번째 진단 시점의 시스템 여유, $S_k = D - \max(\text{최근 응답시간})$. 평균 대신 최근 $n$개 job의 응답시간의 관측 최대를 빼서 계산. 응답시간이 튀면 추정이 늦어질 수 있으므로 검토해야 함 \\
...
\begin{itemize}
\item 진단 신뢰성: 더 긴 window는 더 많은 회전 주기와 주파수 정보를 담아 약한 결함을 더 잘 구분하고, 더 무거운 모델은 결함 구분력이 높다.
...
\begin{itemize}
\item 사용한 FRFconv-TDSNet 모델 [23]은 depthwise convolution 계층이 입력 길이에 비례하는 가변 커널 구조를 가져 $O(W^2)$ 복잡도를 갖는다. 따라서 window를 줄이면 연산량이 선형보다 빠르게 감소한다.
\item 데드라인은 한 window의 수집 시간으로 정의된다: $D = W / f_s$. 비중첩 window에서는 진단 주기 $T$가 곧 $D$와 같다.
...
\begin{itemize}
\item 첫 구현은 M을 고정하고 discrete $(W, H)$만 다룬다. M은 필요성과 보장 조건이 확인된 뒤 확장
...
\toprule
모드 & $W$ & $H$ & 진단 주기 $T = H/f_s$ & 상대 연산량 ($\propto W^2$) & 의미 \\ \midrule
...
\item Offline profiling으로 각 모드의 실행시간을 측정해 실행 가능 여부를 미리 표로 만든다.
\end{itemize}

\subsection{실행시간과 이용률}
\begin{itemize}
\item 각 모드의 추론 실행시간 $C_i$는 단일 값이 아니라 분포로 보고한다. 후보로 평균과 관측 최대(max)를 사용한다.
...
\end{itemize}
\begin{align*}
&\text{실행 가능 모드 집합:}\quad F(k) = \{\, m_i : U_{bg} + C_i(\max)/T_i \le U_{bound} \text{ 그리고 } R_i(\max) \le D \,\} \\
...
\end{itemize}
\item 시스템 여유는 평균이 아니라 최근 응답시간의 관측 최대로 정의한다.
\end{itemize}
\[ S_k = D - \max(\text{최근 응답시간}) \]
...
\item 반면 실제 응답시간은 실행 조건에 따라 runtime에 변하므로, slack $S_k$는 매 진단 시점마다 최근 실제 응답시간 이력(최근 n개 job)으로 계산하고, feasibility 판정에도 이 online 값을 반영한다.
\item 즉 offline profile은 초기 기준값, online 최근 이력은 실시간 보정값이다.
\end{itemize}
\noindent Offline 단계
\begin{enumerate}
\item 후보 모드 집합 $\{m_1, \dots, m_N\}$ 구성 (W 후보 + 물리식 하한 반영)
\item 각 모드의 실행시간 $C_i$ 측정 $\rightarrow$ 평균 / 최대 기록
...
\item M도 연산량을 바꾸는 축이다. 무거운 모델은 정확도가 높지만 실행시간 C가 커진다. 즉 실행시간은 $C = C(W, M)$으로, W와 M이 함께 결정된다.
\item 처리 방안 후보:
\begin{itemize}
\item 첫 구현은 M을 고정해 $C = C(W)$로 두고, W·H의 feasibility와 runtime 선택부터 검증
\item 확장 단계에서는 사전 프로파일한 몇 개의 이산 모델 변형(예: 경량/표준/정밀)을 후보에 추가하고, 각 $(W, M)$ 조합의 $C(W, M)$을 offline에서 측정해 모드를 $(W, H, M)$으로 확장
\item M도 다른 축과 동일하게 feasible 집합 안에서 진단 정확도가 가장 높은 것을 고른다. 무거운 M은 C가 커서 여유가 있을 때만 feasible해짐.
\end{itemize}
\end{itemize}
...
\section{Novelty}
\begin{itemize}
\item 본 연구의 차별점: 외부 상태 트리거나 정확도--실행시간 trade-off 자체의 신규성에 있지 않다. 실시간 DNN 연구는 응용 상태나 system slack에 따라 입력 scale, 모델 복잡도와 실행 depth를 조절하며 [18, 19, 21], mixed-criticality 연구는 physical state와 runtime slack을 결합한다 [24].
\item 다만 조사한 진동 결함 진단 연구에서는, 진동 신호에서 산출한 anomaly score와 runtime 시스템 여유(slack)를 함께 고려하고, 입력 window 길이 W와 진단 간격 H를 분리한 진단 모드 중 deadline을 만족하는 모드를 선택하는 방법은 확인되지 않았다.
\item 본 연구는 anomaly score에 따라 요구되는 진단 빈도와 성능을 반영하고, 시스템 여유를 이용해 실행 가능한 후보를 제한한 뒤 그 안에서 적절한 진단 모드를 선택하는 결합을 대상으로 한다.
\end{itemize}
\noindent 세 가지 구체적 차별점
\begin{enumerate}
\item \textbf{조절 대상}: 기존은 주기·rate·입력 크기 중 하나를 조절한다 [9$\sim$21]. 본 연구는 window, hop, model 세 값을 함께 고르며, 특히 W와 H를 분리해 ``정보량''과 ``진단 빈도''를 독립 변수로 둔다.
...
\textbf{본 연구} & \textbf{O} & \textbf{O} & \textbf{O} & \textbf{O} & \textbf{O} & \textbf{O} \\
\bottomrule
\end{tabular}
\end{center}
...
\item[$\square$] 후보 모드 집합의 W, H 값과 개수 N 확정. 첫 구현은 M 고정.
\item[$\square$] fallback 모드와 전환 hysteresis 정도 확정.
\end{itemize}
\subsection{실험 및 마일스톤}
\begin{itemize}
\item[$\square$] Pi Zero 2W에서 모드별 실행시간 profiling, mode feasibility table 작성. KCC 이용률 표 확장
