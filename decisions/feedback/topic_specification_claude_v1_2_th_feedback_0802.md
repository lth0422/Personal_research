...
- 적응형 입력(S2)과 실시간 DNN(S4)은 서베잉 중이며, 현재까지 확보한 범위만 표로 반영하였음.
...
- 평가 플랫폼은 Raspberry Pi Zero 2W이며, 검증 실험으로 일반 Linux와 PREEMPT_RT 커널의 응답시간 특성을 비교한다.
    - → 검증 실험 이므로 초록에서는 빼고 실험 섹션에서만 언급하세요 본문에 들어갈지 안들어갈지도 모르는데 왜 넣습니까?
...
| `C_i` | 모드 `m_i`의 추론 실행시간. 분포로 보고하며 평균·p99·최대를 사용 → p99가 진짜 실시간시스템 검증에서 많이 쓰이는 수치인지 조사하고 필요 없으면 빼세요 |
...
| `S_k` | k번째 진단 시점의 시스템 여유, `S_k = D − p99(최근 응답시간)`  → p99 마찬가지로 맞는지 조사하고 판단. 최근 응답시간이 왜 p99? |
| `Q(m_i, q_k)` | 현재 기계 상태에서 모드 `m_i`가 주는 진단 효용. 정확도 또는 검출 지연으로 측정 → 진단 효용이 뭐에요…..검출 지연은 또 뭐고요 제발 직역 억지로 하지 말라고요 어려우면 영어로 쓰시던지 |
...
- 회전 기계 진동 결함 진단은 엣지에서 수행할 때 두 요구가 부딪힌다. → trade off 라는 표현이 더 맞지 않아요? 그리고 백그라운드에서 배경 설명이 너무 약한데.. kcc2026 낸 논문 참고해봐요 introduction
...
| # | 논문 | 문제 상황 | 조절 대상 → 뭘 조절하는지 쳐 쓰라고 시발련아 글고 조절 기준이 있으면 뭔데 그게 | 적응·이상감지 판정 기준 → 적응 이상감지는 억지로 번역 하지 말라고 | Deadline | 본 연구와의 차이 |
| --- | --- | --- | --- | --- | --- | --- |
...
- 두 축은 조사를 보강 중이며 현재 확보한 범위만 표로 제시한다. 전체 표는 엑셀 시트 `3_적응형_입력_모델`, `4_RT_DNN`.
...
실시간 DNN 서빙 (어떻게 실시간성을 달성하는가) → 서빙이라는 단어는 왜 붙는거임?
...
- 위의 두개 섹션 나누기 애매한거 아닌가요? 적응형 입력 모델에서 딥러닝 활용 안하는게 있는건가? 그거 써줘봐 표에
- 축 말고 섹션이라는 용어 쓰기
...
- 정식화의 상세는 `manuscript/problem_formulation.md`를 기준으로 한다. → 교수님이 보신다고 생각하고 이런 문구는 지워
...
| 모드 | W | H | 성격 → 뭔 성격 하 미치겠네 이런 표현 국문 논문에서는 아예 안쓴다고요 |
| --- | --- | --- | --- |
| `m_1` | 512 | 512 | 가벼움 |
| `m_2` | 1024 | 512 | 중간 |
| `m_3` | 1024 | 1024 | 중간, 주기 김 |
| `m_4` | 2048 | 2048 | 정밀 |
- 모드 개수 N은 4~6개를 목표로 하고, offline profiling으로 각 모드의 실행시간을 측정해 실행 가능 여부를 미리 표로 만든다. → 어떤 모드인지를 자세히 적으라고 ㅅㅂ 표로
...
- 정책은 실행 가능한 모드 집합 안에서만 진단 효용이 큰 모드를 고른다. → 진단 효용이 Quality임?
...
### 7.4 기계 상태와 이상감지 기준 → 이상 감지는 그냥 anomly detection이라고 적어라. 결함 진단이랑 다른 표현인거 알지?
...
### 7.5 알고리즘 (offline / online 분리) → 확정아니라고 명시 근데 실행시간은 왜 재는거임? 어차피 실제로 온라인에서 시시각각 달라지는데 온라인에서 계산은 못함?
...
- 진단: 정확도 또는 검출 지연.
- 정책 효과: 제안 정책이 static·단일 트리거 baseline 대비 데드라인 miss를 늘리지 않으면서 진단 효용을 개선하는지. → 진단 효용이 정확히 뭔데요 시발….걍 정확도나 맥스 레이턴시나 뭐 수치적으로 제시하던지 하 답답해 진단 효용이라는 표현을 국문에서 첨본다 나는
...
3. **도메인과 보장**: 진동 결함 진단에서 실행 가능한 모드 집합 안에서 선택하고, Pi Zero 2W의 일반 Linux와 PREEMPT_RT에서 실측 응답시간으로 스케줄 가능성을 확인한다. 검토한 FD 문헌이 best-effort에 머문 지점을 데드라인·꼬리 지연과 결합한다. → 도메인과 보장이 맞냐 이 소제목이? 하 다시 검토해

각 축에서 가장 가까운 논문 대비 → 어떤 섹션인데요 미친놈아.. 표로 정리
...
- [22] 이태훈 외, KCC 2026 선행연구 (STM32F407 + Zephyr RTOS 실시간 추론 성능 분석). 정식 서지정보 확정 필요. → papers에 넣어놨으니 확인

당연히 내가 적은 코멘트 다 삭제하고 내 피드백 다 반영하고, 내가 던진 질문들의 답을 문서에 놓기 애매하다면 따로 채팅에 적어라 걍
