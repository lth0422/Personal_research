...
> **0708 면담 반영**: 이 트랙은 현재 독립 논문을 전제로 밀기보다, 방향 2인 `W/H/M` elastic scheduling을 위한 기초 실험 환경과 원인 분석 훈련으로 둔다. 논문화 여부는 실험 결과와 분석 깊이가 충분해진 뒤 판단한다.
...
현재는 위 문장을 확정 기여가 아니라 후보 주장으로 둔다. 교수님 피드백 기준으로는 단순 비교나 synthetic 부하 측정만으로는 약하므로, 센싱, 윈도잉, 추론, 로깅 task가 CPU, memory, I/O 자원을 어떻게 사용하고 왜 latency 또는 miss가 발생하는지 분석해야 한다.
...
주의: 위 novelty는 현재 후보이다. 부하 조건은 임의로 확정하지 않고 PREEMPT_RT/SBC 문헌의 부하 설계를 확인한 뒤 정한다.
...
- 부하 조건 idle/CPU/mem/IO/combined를 그대로 쓸지, 유사 문헌의 부하 설계에 맞춰 조정할지 확인 필요
- 방향 1을 독립 논문으로 밀지, 방향 2의 분석/환경 장으로 흡수할지 결과를 본 뒤 판단 필요
