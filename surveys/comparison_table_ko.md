...
| --- | ---: | --- | --- | --- | --- | --- | --- | --- |
| Choi et al., Low-Cost MCU Shaft FD | 2025 | 회전기계 축 결함 진단 | STM32F401RET6 | Bare metal + X-CUBE-AI | 양자화 (X-CUBE-AI INT8) | sensing, inference, output 실행시간 측정 | 근실시간에 가까움. deadline, jitter, schedulability는 확인되지 않음 | KSC 2025 기반 시스템. KCC Zephyr RTOS 작업의 선행 baseline |
| Thota et al., TinyML Bearing Fault Classification | 2025 | motor bearing fault classification | ESP32 계열 MCU | MCU runtime/RTOS 확인 필요 | TinyML + 양자화 (INT8 추정) | TinyML, quantization, model size와 latency 평가 | best-effort 근실시간. deadline miss와 RTOS scheduling은 확인되지 않음 | 경량화 계열 비교군. 본 연구와 달리 runtime scheduling은 없음 |
| Ma et al., Lightweight Architecture Search FD | 2023 | rotating machinery fault diagnosis | Desktop CPU | 일반 OS, 세부 확인 필요 | NAS (hardware-aware, FLOP/latency 목표) | architecture search에 계산 시간 objective 포함 | best-effort 근실시간. per-task deadline이나 RTOS는 확인되지 않음 | `M` 후보를 offline으로 가볍게 설계하는 비교군 |
| Lee and Kim, FRFconv-TDSNet | 2024 | vibration machine fault diagnosis | Raspberry Pi 4B SoC/SBC | Linux 계열, RT extension 확인 필요 | 경량 아키텍처 수동 설계 (FRFconv + TDS 블록) | edge inference time 평가 | edge inference 가능성은 보이나 deadline scheduling은 없음 | KCC 모델 배경, noise robustness, `M` 후보 |
| Jalonen et al., Real-Time Vibration-Based Bearing FD | 2024 | time-varying speed bearing FD | MacBook Pro M1 Pro | Desktop OS, 세부 확인 필요 | 알고리즘 경량화 (시변속도 보상 처리) | acquisition duration 대비 processing time 평가 | 실시간 processing 가능성 평가. RTOS/deadline miss는 없음 | `W` 또는 segment length가 real-time processing과 연결된 사례 |
...
| He et al., Improved Cyclostationary Edge FD | 2023 | acoustic motor-bearing diagnosis | STM32F407 | OS/RTOS 미명시 | 알고리즘 경량화 (개선된 cyclostationary 분석, 비DL) | pipeline step별 실행시간 측정 | 전체 10.294 s로 acquisition 1 s보다 김; deadline guarantee 없음, `B` | "online/real-time"과 deadline 충족을 구분하는 반례 |
| Pubalan et al., Simulated 1D-CNN RT-FDD | 2025 | bearing multi-class diagnosis | simulated replay, hardware 미명시 | 실행 환경 미명시 | 경량 1D-CNN (실 배포 없음, 시뮬레이션) | one-revolution input과 0.03 s prediction | 실제 device/deadline/tail 없음; `B` | 물리 회전으로 `W`를 정하지만 runtime adaptation은 아님 |
| Arciniegas et al., TinyML Motor Vibration | 2025 | motor anomaly alerting | XIAO ESP32S3 | MCU runtime 미명시 | TinyML (Edge Impulse 배포) | 25 ms inference, 약 300 ms sensing-to-alert | 평균 latency만 있음; `B` | End-to-end pipeline과 network interference 분해 참고 |
| Gupta and Shivhare, Embedded TinyML ESP32 | 2025 | four-class vibration FD | ESP32 | MCU runtime 미명시 | 양자화 (INT8/FP32, TFLite) | 두 model의 on-device latency 비교 | deadline/tail/miss 없음; `B` | Fixed `W=256`, `H=128`, model profile 비교. adaptive sampling 구현은 확인되지 않음 |
| Lima, Edge Impulse Motor FD | 2025 | broken rotor bar detection | nRF52840 Arduino Nano 33 BLE | MCU runtime 미명시 | 양자화 (INT8/FP32) + Edge Impulse 자동 NAS | W/rate/model/quantization별 target latency | deadline/tail/miss 없음; `B` | `W/H/M` 후보 profile과 subtle-fault utility trade-off 근거 |
...
| Zhan et al., APTL-net | 2026 | edge bearing diagnosis | Jetson Xavier NX | Linux/JetPack/TensorRT | Adaptive pruning + DW conv + BN fusion (Triple-lightweight) | preprocessing 포함 average latency와 online test | deadline/tail/miss 없음; `B` | SoC/Linux `M` profile. Adaptive pruning은 training-time |
...
| Langarica et al., Industrial Internet FD | 2020 | industrial motor staged diagnosis | Siemens IoT2040/CMS2000 + server | IoT edge/server stack | 없음 (signal processing cascade) | 1 Hz DIPCA/RBC 뒤 vibration CNN cascade | stage latency/deadline 없음; `B` | Machine evidence가 expensive stage를 trigger하지만 slack은 보지 않음 |
| Shan et al., CS-DKELM | 2022 | rotating machinery fault diagnosis | Zynq UltraScale+ MPSoC | Linux | 압축 센싱 (sparse 표현) + DKELM (비DL 커널) | compressed sampling과 average diagnosis time | physical average 0.17 s로 100 ms 주장과 불일치; `B` | Input compression을 fidelity-cost mode로 보는 SoC 비교군 |
| Sayghe, Physics-Aware Transformer | 2026 | variable-condition bearing FD | Raspberry Pi 4 | Linux/ONNX Runtime | 물리 인식 경량 Transformer (patch 수 최소화) + ONNX Runtime | physics-guided patch, average 43.6 ms inference | deadline/tail/miss 없음; `B` | Fault physics가 `W` 후보의 최소 정보량을 제한한다는 직접 근거 |
